  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const imgs: string[] = Array.isArray(rawRes.data?.data?.images)
          ? rawRes.data.data.images
          : [];
        setImages(imgs);
        setCurrent(0);
      } finally {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-foreground">
            ...
            >
              {p?.quantity ?? 0}
              {(p?.quantity ?? 0) === 0 && (
                <div className="ml-2 text-xs bg-red-100 text-red-700 px-2 rounded">
                  Out of Stock
                </div>
              )}
              {(p?.quantity ?? 0) > 0 &&
                (p?.quantity ?? 0) <= (p?.minStock ?? 0) && (
                  <div className="ml-2 text-xs bg-yellow-100 text-yellow-800 px-2 rounded">
                    Low Stock
                  </div>
                )}
            </div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            {p?.profitMargin !== undefined && (
              <div className="flex items-center gap-2">
                Profit Margin:
                <div
                  className={`flex items-center px-2 py-0.5 rounded text-s font-semibold ${
                    p.profitMargin > 0
                      ? "bg-green-100 text-green-700"
                      : "bg-red-100 text-red-700"
                  }`}
                >
                  {Number(p.profitMargin).toFixed(2)}%
                </div>
              </div>
            )}
          </div>
            ...
            <div className="flex items-center gap-4 border p-2 rounded text-foreground">
              {Array.from({ length: 5 }).map((_, index) => (
                <Star
                  key={index}
                  className={`w-8 h-8 ${
                    Number(p?.statistics?.avgRating ?? 0) > index
              ...
              <span className="text-sm font-semibold text-foreground">
                {Number(p?.statistics?.avgRating ?? 0).toFixed(1)}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <div className="text-foreground font-semibold">
                ${Number(p?.statistics?.totalRevenue ?? 0).toLocaleString()}
              </div>
            </div>
            <div>
              <div className="text-muted-foreground">Avg Rating</div>
              <div className="text-foreground font-semibold">
                {p?.statistics?.avgRating ?? "--"}
              </div>
            </div>
            <div>
              <div className="text-muted-foreground">Total Ratings</div>
              <div className="text-foreground font-semibold">
                {p?.statistics?.totalRatingCount ?? 0}
              </div>
            </div>
            <div>
          ...
          <div className="flex items-center gap-2 text-foreground">
            {Array.from({ length: 5 }).map((_, index) => (
              <Star
                ...
                onClick={() => handleRateProduct(index + 1)}
              />
            ))}
          </div>
        </div>
